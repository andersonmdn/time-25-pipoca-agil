import { z } from 'zod'
import { idInt, optionalTrimSchema, requireAtLeastOneField, urlArrSchema } from '../shared/helpers'

export const stationReviewSchema = z.object({
  userId: idInt,
  stationId: idInt,
  rating: z.coerce.number().int().min(1).max(5),
  title: optionalTrimSchema,
  comment: optionalTrimSchema,
  photos: urlArrSchema,
})
...
export const stationReviewUpdateSchema = stationReviewSchema.partial().superRefine(requireAtLeastOneField())
